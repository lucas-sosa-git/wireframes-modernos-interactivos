(function () {
  ...
  function createInstance(target, options) {
    ...
    function actions(record) {
      if (options.actions === "dun14-selection") return `${button("create-dun14", record, "Generar nueva unidad de despacho", "", "btn-primary product-table-generate-dun14")}${button("detail", record, "Detalle", "eye")}`;
  ...
  function iconMarkup(name) {
    if (name.includes(".")) return `<img class="action-image" src="${escapeAttr(name)}" alt="">`;
    const icons = {
      eye: '<path d="M16 8s-3-5.5-8-5.5S0 8 0 8s3 5.5 8 5.5S16 8 16 8M1.173 8a13 13 0 0 1 1.66-2.043C4.12 4.668 5.88 3.5 8 3.5s3.879 1.168 5.168 2.457A13 13 0 0 1 14.828 8c-.058.087-.122.183-.195.288-.335.48-.83 1.12-1.465 1.755C11.879 11.332 10.119 12.5 8 12.5s-3.879-1.168-5.168-2.457A13 13 0 0 1 1.172 8z"/><path d="M8 5.5a2.5 2.5 0 1 0 0 5 2.5 2.5 0 0 0 0-5"/>',
      files: '<path d="M13 0a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2h-1v1a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h1V2a2 2 0 0 1 2-2zM4 3v8a2 2 0 0 0 2 2h5V3a1 1 0 0 0-1-1H5a1 1 0 0 0-1 1m8 8h1a1 1 0 0 0 1-1V2a1 1 0 0 0-1-1H6a1 1 0 0 0-1 1v1h5a2 2 0 0 1 2 2z"/>',
      "pencil-square": '<path d="M15.502 1.94a.5.5 0 0 1 0 .706l-1.793 1.793-2.647-2.647L12.855.5a.5.5 0 0 1 .707 0z"/><path d="M1 13.5V16h2.5l7.372-7.372-2.647-2.647z"/><path fill-rule="evenodd" d="M1 1h11.5a1 1 0 0 1 1 1V6l-1 1V2a.5.5 0 0 0-.5-.5H1.5A.5.5 0 0 0 1 2v12a.5.5 0 0 0 .5.5H8l-1 1H1.5A1.5 1.5 0 0 1 0 14V2A1.5 1.5 0 0 1 1 1"/>',
      "clock-history": '<path d="M8.515 3.879a.5.5 0 0 0-1 0v4.182l3.182 1.909a.5.5 0 1 0 .516-.857L8.515 7.494z"/><path d="M8 3a5 5 0 1 0 4.546 2.914.5.5 0 0 1 .91-.414A6 6 0 1 1 8 2z"/><path d="M8 1.5A.5.5 0 0 1 8.5 1h1a.5.5 0 0 1 0 1h-.5v1a.5.5 0 0 1-1 0V2H7.5a.5.5 0 0 1 0-1z"/>',
      image: '<path d="M.5 1A1.5 1.5 0 0 0-1 2.5v11A1.5 1.5 0 0 0 .5 15h15a1.5 1.5 0 0 0 1.5-1.5v-11A1.5 1.5 0 0 0 15.5 1zm0 1h15a.5.5 0 0 1 .5.5v6.248l-3.37-3.37a1 1 0 0 0-1.415 0L6.5 10.293 4.354 8.146a.5.5 0 0 0-.708 0L0 11.793V2.5A.5.5 0 0 1 .5 2m15 12H.5a.5.5 0 0 1-.5-.5v-.293l4-4 2.146 2.147a1 1 0 0 0 1.415 0l4.793-4.793L16 10.207V13.5a.5.5 0 0 1-.5.5M4.5 4a1.5 1.5 0 1 0 0 3 1.5 1.5 0 0 0 0-3"/>'
    };
    return `<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="currentColor" viewBox="0 0 16 16" aria-hidden="true">${icons[name] || ""}</svg>`;
  }

  function getRecords(mode) { if (!window.GS1ProductCatalog) return []; return mode === "dispatchUnits" ? window.GS1ProductCatalog.getDispatchUnits().map((record) => ({ ...record, type: "DUN 14", status: record.status || "Activo" })) : window.GS1ProductCatalog.getCommercialProducts(); }
